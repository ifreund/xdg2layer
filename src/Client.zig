...
fn handleGlobal(
    ...
    if (std.cstr.cmp(interface.?, @ptrCast([*:0]const u8, c.wl_shm_interface.name.?)) == 0) {
    ...
    } else if (std.cstr.cmp(interface.?, @ptrCast([*:0]const u8, c.wl_drm_interface.name.?)) == 0) {
        @import("drm.zig").init(self.server.wl_display, wl_registry.?, name, version);
        ...
        self.server.compositor.init(self.server, wl_compositor, version);
