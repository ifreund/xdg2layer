...
pub fn init(self: *Self, wl_callback: *c.wl_buffer, wl_resource: *c.wl_resource) void {
...
fn eventDone(data: ?*c_void, wl_callback: *c.wl_callback) callconv(.C) void {
    const self = @intToPtr(*Self, @ptrToInt(data));
    c.wl_callback_send_done(self.wl_resource);
